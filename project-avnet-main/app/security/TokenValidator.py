...
class TokenValidator:
    ...
    def __call__(self, credentials: HTTPAuthorizationCredentials = Depends(security), db: Session = Depends(get_db)):
        ...
        if not payload:
            ...
            LoggerManager.get_logger().warning("Invalid token received in request header: %s", token)
            raise HTTPException(
                status_code=401, 
        ...
        if user_role not in self.allowed_roles:
            LoggerManager.get_logger().warning("User %s:%s with role %s not allowed to access this endpoint", user_s_id, user_UUID, user_role)
        ...
        if not user:
            LoggerManager.get_logger().warning("User with UUID %s not found in database", user_UUID)
            raise HTTPException(
                status_code=404, 
                detail="User not exists"
            )
